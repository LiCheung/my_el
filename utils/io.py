...
import json
import pickle
from config import SUBMIT_DIR, PROCESSED_DATA_DIR, ENTITY_ID_FILENAME
...
def submit_result(submit_file, test_data, pred_result):
    for testdata, predresult in zip(test_data, pred_result):
        print(1111111111222)
        print(testdata)
        print(predresult)
    ...
    try:
        ...
        with open(os.path.join(SUBMIT_DIR, submit_file), 'w', encoding="utf-8") as writer:
            for i in range(len(test_data)):
                text_id = test_data[i]['text_id']
                text = test_data[i]['raw_text']
                for x in pred_result[i]:
                    print(111111)
                    print(x)
